...
- **Race condition**: Shared variable accessed from multiple goroutines without synchronization. Run `go test -race`. Mutex-guarded reads protect writers but: if a getter holds `RLock` and a setter holds `Lock`, concurrently calling both = data race (RLock allows concurrent reads but writes need exclusive Lock).
